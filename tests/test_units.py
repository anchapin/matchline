...
from datasets_adapter import (polygon_area_px2, ScheduleEntry, Detection,
                              rollup_takeoff)
from registration import (Affine2D, Facade, register_elevation_grid,
                          register_elevation_geometric, point_in_polygon,
...
def test_polygon_area_square():
    assert polygon_area_px2([(0, 0), (20, 0), (20, 30), (0, 30)]) == \
        pytest.approx(600.0)
...
def test_affine_scale_translate_roundtrip():
    aff = Affine2D.from_scale_translate(px_per_m=50.0, ox_px=120.0,
                                        oy_px=150.0)
...
def test_facade_registration_grid():
    fac = Facade(name="south", ref_corner_m=(0.0, 15.0), length_m=24.0,
                 fixed_coord_m=15.0, axis="x")
    reg = register_elevation_grid(
        "elev_A201", fac, plan_grid_m={"A": 0.0, "B": 8.0, "C": 16.0,
                                      "D": 24.0},
        elev_bubbles=[{"label": "A", "u_px": 60.0},
                      {"label": "B", "u_px": 380.0},
                      {"label": "C", "u_px": 700.0},
                      {"label": "D", "u_px": 1020.0}],
        v_ground_px=800.0, elev_px_per_m=40.0, revision=1)
...
def test_facade_registration_geometric():
    ...
    reg = register_elevation_geometric(
        "elev_A202", fac, wall_u0_px=60.0, elev_px_per_m=40.0,
        v_ground_px=800.0, revision=1)
...
def test_rollup_takeoff_count_x_dims():
    sched = {"A": ScheduleEntry(tag="A", category="window",
                                width_m=1.5, height_m=1.2)}
    dets = [Detection(label="Window", tag="A", score=1.0,
                      bbox=(0, 0, 10, 10), source="t",
                      drawing_type="elevation") for _ in range(4)]
...
def test_rollup_unmatched_reported_not_dropped():
    sched = {}
    dets = [Detection(label="Window", tag="ZZZ", score=1.0,
                      bbox=(0, 0, 10, 10), source="t")]
...
def test_lpd_rollup_matches_schedule(bldg_3room):
    ...
    for sid, sp in model.spaces.items():
        ...
        if sp.area_m2:
            assert sp.lighting.lpd_w_m2 == pytest.approx(
                exp / sp.area_m2)
...
def test_simplify_ring_respects_budget():
    ...
    for i in range(4):
        ...
        for k in range(per_edge):
            t = k / per_edge
            ring.append((x0 + (x1 - x0) * t + rng.uniform(-0.05, 0.05),
