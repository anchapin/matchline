...
from classes import AEC_MAP, CLASS_IDS
...
def main():
    ...
    ap.add_argument(
        "--src", default=os.path.expanduser("~/workspace/datasets/aec-geometric-bench/dataset")
    )
    ap.add_argument("--out", default=os.path.expanduser("~/workspace/datasets/detector_yolo/aec"))
    ...
    tree = lxml.etree.parse(
        os.path.join(args.src, "annotations_15_scoring_ready.xml"), safe_xml_parser()
    )
    stats = Counter()
    for im in tree.getroot():
        ...
        for el in im:
            ...
            if uni == "UNKNOWN":
                ...
                continue
            x0 = float(el.attrib["xtl"])
            y0 = float(el.attrib["ytl"])
            x1 = float(el.attrib["xbr"])
            y1 = float(el.attrib["ybr"])
