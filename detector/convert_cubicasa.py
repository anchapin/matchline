...
def convert_plan(svg_path):
    ...
    if vb:
        ...
        vbw = float(root.get("width", 0))
        vbh = float(root.get("height", 0))
    out = []
    ...
    def walk(el, mat, hidden):
        ...
        if (
            ...
            and not hidden
            and not any(
                d.tag.split("}")[-1] == "g" and d.get("class", "") in TARGET
                for d in el.iter()
                if d is not el
            )
        ):
...
def main():
    ...
    ap.add_argument(
        "--src", default=os.path.expanduser("~/workspace/datasets/cubicasa5k/cubicasa5k")
    )
    ap.add_argument(
        "--out", default=os.path.expanduser("~/workspace/datasets/detector_yolo/cubicasa")
    )
