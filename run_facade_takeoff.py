...
from facade_takeoff import (CLASS_NAMES, dataset_priors, facade_takeoff,
                            iter_facades, load_mask, parse_xml_boxes,
                            sweep, xml_agreement)
...
def demo_facades(n: int = 5) -> None:
    ...
    for i, (fid, png, xml) in enumerate(facs):
        ...
        w = ag["per_class"][3]
        print(f"  XML vs mask glazing: area_ratio={g['ratio_xml_over_mask']:.2f} "
              f"iou={g['iou']:.2f} "
              f"recall_slop={g['coverage_mask_in_xml']:.2f} "
              f"prec_slop={g['coverage_xml_in_mask']:.2f}")
        print(f"  windows: {w['n_xml_boxes']} XML boxes vs "
              f"{w['n_mask_components']} mask components")
        env = t.to_envelope_dict()
        print(f"  envelope handoff: wwr={env['wwr']:.3f} "
              f"scaled={env['scale_supplied']}")
...
def full_sweep() -> dict:
    ...
    print("=" * 70)
    t0 = time.time()
    takeoffs, agreements, skipped = sweep(DATA_ROOT, with_xml=True)
    dt = time.time() - t0
    print(f"sweep: {len(takeoffs)} ok, {len(skipped)} skipped, "
    ...
    def row(name, st):
        return (f"  {name:16s} n={st['n']:3d}  mean={st['mean']:.3f}  "
    ...
    def agg(key):
        vs = [a["glazing_combined"][key] for a in agreements
              if a["glazing_combined"][key] is not None]
        vs = np.array(vs)
        return (f"mean={vs.mean():.3f} median={np.median(vs):.3f} "
    ...
    for lab in (3, 4, 8, 12):
        vs = np.array([a["per_class"][lab]["ratio_xml_over_mask"]
                       for a in agreements
                       if a["per_class"][lab]["ratio_xml_over_mask"] is not None])
        print(f"  {CLASS_NAMES[lab]:8s} n={len(vs):3d} "
    ...
    out = {
        ...
        "priors": priors,
        "notes": ("Fractions are over the wall-plane pixel set "
