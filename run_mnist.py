...
X = np.load("data/mnist_X.npy").astype(np.float64)   # (70000, 784)
y = np.load("data/mnist_y.npy").astype(np.int64)
...
for name, fn in [("sum", clf.predict_sum), ("logodds", clf.predict_logodds)]:
    ...
    print(f"{name}: acc={acc*100:.2f}%  infer={t_inf/len(Xte)*1e6:.2f} us/img", flush=True)
...
print("saved mnist_results.json")
