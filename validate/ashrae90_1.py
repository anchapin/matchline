...
_WALL_U_MAX_BTU: dict[tuple[str, str], float] = {
    ...
    ("*", "1A"): 0.58, ("*", "1B"): 0.58,
    ("*", "2A"): 0.58, ("*", "2B"): 0.58,
    ("*", "3A"): 0.58, ("*", "3B"): 0.58, ("*", "3C"): 0.58,
    # Temperate (4-5)
    ("*", "4A"): 0.40, ("*", "4B"): 0.40, ("*", "4C"): 0.40,
    ("*", "5A"): 0.40, ("*", "5B"): 0.40, ("*", "5C"): 0.40,
    # Cold (6-8)
    ("*", "6A"): 0.35, ("*", "6B"): 0.35,
    ("*", "7"): 0.35, ("*", "8"): 0.35,
}
...
_ROOF_U_MAX_BTU: dict[str, float] = {
    "1A": 0.360, "1B": 0.360,
    "2A": 0.360, "2B": 0.360,
    "3A": 0.360, "3B": 0.360, "3C": 0.360,
    "4A": 0.282, "4B": 0.282, "4C": 0.282,
    "5A": 0.282, "5B": 0.282, "5C": 0.282,
    "6A": 0.248, "6B": 0.248,
    "7": 0.248, "8": 0.248,
}
...
_WINDOW_SHGC_MAX: dict[str, float] = {
    "1A": 0.25, "1B": 0.25,
    "2A": 0.25, "2B": 0.25,
    "3A": 0.25, "3B": 0.25, "3C": 0.60,
    "4A": 0.40, "4B": 0.40, "4C": 0.60,
    "5A": 0.40, "5B": 0.40, "5C": 0.60,
    "6A": 0.40, "6B": 0.40,
    "7": 0.40, "8": 0.40,
}
...
def _check_wall_u_factor(ctx: _Ctx) -> CheckResult:
    ...
    for wall in getattr(model, "envelope", []):
        ...
        if u_actual > u_max:
            violations.append(
                f"{wall.id}: {u_actual:.3f} > {u_max:.3f} Btu/h·ft²·°F"
            )
