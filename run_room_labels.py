...
def main():
    ...
    for lb in labeled.labels:
        print(
            f"  raw={lb.raw_text!r:22} -> name={lb.name!r:14} number={lb.number!r:6} "
    ...
    cases = [
        ("OPEN OFFICE 201", ("OPEN OFFICE", "201")),
        ("201", ("", "201")),
        ("201A", ("", "201A")),
        ("CONF 202", ("CONF", "202")),
    ...
    gap_label = rl.RoomLabel(
        name="VEST",
        number="300",
        raw_text="VEST 300",
        bbox=(1180, 900, 1220, 930),
        centroid=(1200, 915),
        ocr_confidence=0.9,
        parse_confidence=1.0,
    )
    units = [rl.LabeledSpace(polygon_px=s.polygon_px) for s in spaces]
    rl.assign_labels(units, [gap_label])
    a, b = units[8], units[9]
    got = (
        "room8"
        if (a.name, a.number) == ("VEST", "300")
        else "room9"
        if (b.name, b.number) == ("VEST", "300")
        else "none"
    )
    src = (a if got == "room8" else b).label_source if got != "none" else ""
    print(
        f"nearest-fallback unit test: assigned to {got} via '{src}' "
