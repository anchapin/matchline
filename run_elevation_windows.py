...
from synth.multidiscipline import generate_building, SILL_M  # noqa: E402
from synth.sheets import SCHED_BY_TAG  # noqa: E402
from elevation_windows import (  # noqa: E402
    build_model_with_elevations, detect_windows, observations_to_facade,
    merge_elevation_observations, reconcile_window_counts,
    assign_window_tag, DaylightParams)
from registration import (  # noqa: E402
    Facade, register_elevation_grid, register_elevation_geometric,
    point_in_polygon)
...
def _gt_windows_m(bldg):
    ...
    for i, w in enumerate(bldg["south_windows"]):
        ...
        out[f"W{i + 1}"] = {
            ...
            "width": w["s1_m"] - w["s0_m"],
            "sill": SILL, "head": SILL + h,
            "tag": w["tag"], "room": w["room_number"]}
    return out
...
def _register(bldg, key):
    ...
    if data.get("bubbles"):
        reg = register_elevation_grid(
            ...
            elev_bubbles=data["bubbles"], v_ground_px=data["v_ground_px"],
...
def score_detection(bldg):
    ...
    for key in ("elev_grid", "elev_nogrid"):
        ...
        for wid, g in gt.items():
            ...
            f = fw[bj]
            matched += 1
            errs.append((abs(f.s_center_m - g["center"]),
                         abs(f.width_m - g["width"]),
                         abs(f.sill_m - g["sill"]),
                         abs(f.head_m - g["head"])))
            win_sched, _ = link._schedules(bldg)
            tag_ok += (assign_window_tag(f.width_m, f.height_m, win_sched)
                       == g["tag"])
        fp = len(fw) - matched
        rows[key] = {"detected": len(obs), "gt": len(gt),
                     "matched": matched, "fp": fp,
                     "tag_ok": tag_ok, "errs": errs}
    return rows
...
def _dist_to_poly(pt, poly):
    ...
    for i in range(n):
        ...
        bx, by = poly[(i + 1) % n]
        abx, aby = bx - ax, by - ay
        t = ((pt[0] - ax) * abx + (pt[1] - ay) * aby) / max(abx**2 + aby**2,
                                                           1e-12)
        t = max(0.0, min(1.0, t))
    ...
    for sid, sp in model.spaces.items():
        ...
        for z in list(sp.daylight.primary) + list(sp.daylight.secondary):
            ...
            if z.zone_class == "primary":
                exp = min(params.primary_depth_factor * H,
                          max(D - v[1] for v in poly))
            else:
                exp = min(params.secondary_depth_factor * H,
                          max(D - v[1] for v in poly))
            if abs(max(depths) - exp) > 0.06:
                bad.append(f"{z.id}: max depth {max(depths):.2f} != "
                           f"expected {exp:.2f}")
    return n_p, n_s, bad
...
def test_reconciliation_flags():
    ...
    merged = [
        MergedWindow(id="MW-1", facade="south", s0_m=1, s1_m=2.2,
                     s_center_m=1.6, width_m=1.2, sill_m=0.9, head_m=2.4,
                     height_m=1.5, tag="A", category_hint="window"),
        MergedWindow(id="MW-2", facade="south", s0_m=3, s1_m=4.2,
                     s_center_m=3.6, width_m=1.2, sill_m=0.9, head_m=2.4,
                     height_m=1.5, tag="A", category_hint="window"),
        MergedWindow(id="MW-3", facade="south", s0_m=5, s1_m=6.2,
                     s_center_m=5.6, width_m=1.2, sill_m=0.9, head_m=2.4,
                     height_m=1.5, tag="A", category_hint="window"),
        MergedWindow(id="MW-4", facade="south", s0_m=7, s1_m=8.2,
                     s_center_m=7.6, width_m=1.2, sill_m=0.9, head_m=2.4,
                     height_m=1.5, tag="A", category_hint="window"),
        MergedWindow(id="MW-5", facade="south", s0_m=9, s1_m=10.8,
                     s_center_m=9.9, width_m=1.8, sill_m=0.9, head_m=2.4,
                     height_m=1.5, tag="B", category_hint="window"),
        MergedWindow(id="MW-6", facade="south", s0_m=11, s1_m=12.0,
                     s_center_m=11.5, width_m=1.0, sill_m=0.9, head_m=2.4,
                     height_m=1.5, tag=None, category_hint="window"),
    ]
    flags = reconcile_window_counts(model, plan_counts, merged, "south",
                                    "arch_A101")
    kinds = [i.kind for i in model.review_queue]
    ok = (len(flags) == 3
          and kinds.count("window_reconciliation") == 3
          and any("1 'B'" in f["description"] and "no elevation instance"
                  in f["description"] for f in flags)
          and any("'A'" in f["description"] and "no matching arch-plan"
                  in f["description"] for f in flags)
...
def test_dedup_conflict():
    ...
    from elevation_windows import FacadeWindow, merge_elevation_observations
    from building_model import Provenance
    a = FacadeWindow(id="s1:EW-1", sheet_id="s1", facade="south",
                     s0_m=5.0, s1_m=6.2, s_center_m=5.6, width_m=1.2,
                     sill_m=0.9, head_m=2.4, height_m=1.5,
                     category_hint="window", confidence=0.8,
                     provenance=Provenance(sheet_id="s1", revision=1,
                                           method="x", confidence=0.8))
    b = FacadeWindow(id="s2:EW-1", sheet_id="s2", facade="south",
                     s0_m=5.5, s1_m=6.7, s_center_m=6.1, width_m=1.2,
                     sill_m=0.9, head_m=2.4, height_m=1.5,
                     category_hint="window", confidence=0.8,
                     provenance=Provenance(sheet_id="s2", revision=1,
                                           method="x", confidence=0.8))
    merged, conflicts = merge_elevation_observations([[a], [b]],
                                                     tol_m=0.30)
    # centers differ by 0.5 > tol, intervals overlap -> conflict
    return (len(merged) == 2 and len(conflicts) == 1
            and conflicts[0]["delta_center_m"] == 0.5)
...
def main():
    ...
    for seed, span in [(201, False), (202, True), (203, False)]:
        ...
        for key, r in det.items():
            ...
            ok = (r["matched"] == r["gt"] and r["fp"] == 0
                  and r["tag_ok"] == r["gt"])
            all_ok &= ok
            print(f"  [{key}] detected {r['detected']}/{r['gt']} GT, "
                  f"matched {r['matched']}, FP {r['fp']}, "
                  f"tag_ok {r['tag_ok']}/{r['gt']}")
            print(f"    center err mean {mc*100:.1f} cm max {mx*100:.1f} cm; "
                  f"width {mw*100:.1f} cm; sill {ms*100:.1f} cm; "
                  f"head {mh*100:.1f} cm   {'OK' if ok else '<-- FAIL'}")

        model, report = build_model_with_elevations(
            bldg, daylight_params=params)
        print(f"  [dedup] merged {report.n_merged} (GT {len(gt)}), "
        ...
        ok = (report.n_merged == len(gt) and report.n_conflicts == 0
        ...
        for sid, sp in model.spaces.items():
            for o in sp.openings:
                g = next((v for v in gt.values()
                          if v["room"] == sp.number
                          and abs(v["center"] - o.s_center_m) < 0.5), None)
                if g:
                    errs.append(abs(g["center"] - o.s_center_m))
        if errs:
            print(f"  [attach] {len(errs)}/{len(gt)} openings matched to GT "
        ...
        placed = [o for sp in model.spaces.values() for o in sp.openings
                  if o.s_center_m is not None and o.head_m is not None]
        print(f"  [placement] {len(placed)}/{len(gt)} openings carry "
              f"s_center_m + head_m")
        all_ok &= len(placed) == len(gt)

        n_p, n_s, bad = check_daylighting(model, bldg, params)
        print(f"  [daylight] primary zones {n_p}, secondary zones {n_s}; "
        ...
        rt = (len(m2.spaces) == len(model.spaces)
              and all(len(m2.spaces[s].daylight.primary)
                      == len(model.spaces[s].daylight.primary)
