...
sys.path.insert(0, str(Path.home() / "workspace" / "vendor" / "pylibs"))
...
from geometry_simplify import (footprint_from_regions, simplify_ring,
                               simplify_report)
from bem_export import (model_from_takeoff, write_gbxml, validate_gbxml,
                        write_ifc4, validate_ifc4)
...
def main(sheet_id: str = "sheet_007"):
    ...
    gt = json.load(open(HERE / "synth" / "out" / "sheets" / f"{sheet_id}.gt.json"))
    s = 1.0 / gt["scale_px_per_m"]
    src = f"synth:{sheet_id}"

    # 1-3: detections (GT tags) -> schedule -> rollup
    dets = [Detection(label=x["type"], tag=x["tag"], score=1.0,
                      bbox=tuple(x["bbox_px"]), source=src,
    ...
    spaces = [LabeledSpace(polygon_px=r["polygon_px"], source=src)
              for r in gt["rooms"]]
    ...
    # 6: BEM model + exports
    model = model_from_takeoff(takeoff, labeled, sres,
                               wall_height_m=WALL_H_M,
                               building_name=f"Jesse Demo {sheet_id}")
    ...
    print(f"spaces: {len(model.spaces)} "
          f"(labeled {labeled.n_labeled}/{labeled.n_total})")
    print(f"gbXML surfaces: {n_surf} "
          f"(walls {len(model.ring_m)} + roof + slab)")
    print(f"gbXML openings: {n_open} "
          f"(windows {sum(1 for o in model.openings if o.category == 'window')}, "
          f"doors {sum(1 for o in model.openings if o.category == 'door')})")
    print(f"envelope area delta: {rep['area_delta_pct']:+.3f}% "
          f"(tol {rep['tolerance_pct']:.1f}%, "
          f"surface reduction {rep['reduction_pct']:.0f}%)")
    print(f"floor area total: {sum(sp.area_m2 for sp in model.spaces):.1f} m2 "
