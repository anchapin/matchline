...
except ImportError:  # fall back to the workspace-vendored copy
    import sys as _sys
    from pathlib import Path as _Path
    _vendor = _Path.home() / "workspace" / "vendor" / "pylibs"
...
def _tri_area(a, b, c) -> float:
    ...
    return 0.5 * abs((c[0] - a[0]) * (b[1] - a[1])
                     - (c[1] - a[1]) * (b[0] - a[0]))
...
def simplify_ring(ring, tol: float = 0.02, min_feature: float | None = None,
                  wall_height: float | None = None,
                  dp_eps: float | None = None,
                  max_single_step: float = 0.0025) -> SimplifyResult:
    ...
    if n0 < 4:
        ...
        return SimplifyResult(ring, n0, n0, a, a, 0.0, tol,
                              provenance=[{"surface": i, "from": [i],
                                           "note": "unchanged (too few vertices)"}
                                          for i in range(n0)])
    ...
    if dp_eps:
        ...
        if len(new_work) >= 3:
            ...
            if trial.is_valid:
                d_area = (abs(envelope_area(new_work, wall_height) - orig_area)
                          / orig_area)
                if d_area <= tol:
                    work, orig_idx = new_work, new_idx
                else:
                    skipped.append({"op": "douglas_peucker",
                                    "reason": f"area delta {d_area:.3%} > "
                                              f"tol {tol:.1%}"})
            else:
                skipped.append({"op": "douglas_peucker",
    ...
    single_cap = (min_feature if min_feature is not None
                  else max_single_step) * orig_area
    ...
    while heap and alive_count > 4:
        ...
        if c > single_cap:
            if i not in over_cap:
                over_cap.add(i)
                skipped.append({"op": "vertex_removal",
        ...
        if _seg_intersects_ring(work[p], work[q], ring_pts, p, i):
            skipped.append({"op": "vertex_removal",
                            "reason": f"vertex {orig_idx[i]}: removal would "
                                      f"self-intersect"})
    ...
    if not poly.is_valid or len(new_ring) < 3:
        skipped.append({"op": "final",
                        "reason": "simplified ring invalid; returning original"})
        return SimplifyResult(ring, n0, n0, orig_area, orig_area, 0.0, tol,
                              provenance=[{"surface": i, "from": [i],
                                           "note": "rollback: invalid result"}
                                          for i in range(n0)],
                              skipped=skipped, valid=False)
    ...
    for s in range(m):
        ...
        note = ("merged %d source segments" % len(src_edges)
                if len(src_edges) > 1 else "unchanged")
        provenance.append({"surface": s, "from": src_edges, "note": note})

    return SimplifyResult(
        ring=new_ring, original_count=n0, simplified_count=m,
        original_area=orig_area, simplified_area=new_area,
        area_delta_pct=delta * 100.0, tol=tol, provenance=provenance,
        skipped=skipped, valid=True)
...
def simplify_report(res: SimplifyResult) -> dict:
    ...
    return {
        ...
        "reduction_pct": (100.0 * (res.original_count - res.simplified_count)
                          / res.original_count) if res.original_count else 0.0,
