...
from synth.lighting import CLASS_TO_TAG, LIGHTING_GLYPHS, LIGHTING_SCHEDULE
from synth.lighting_sheets import DEFAULT_PLAN, ROOM_FIXTURE_PLAN
from synth.mech import _draw_diffuser, _draw_sensor, _draw_vav
from synth.sheets import ROOM_NAMES, SCHED_BY_TAG, SCHEDULE, _font
...
def _subdivide_strip(x0, x1, y0, y1, n, rng):
    ...
    if any(bounds[k + 1] - bounds[k] < MIN_ROOM_M - 1e-6 for k in range(n)):
...
def _layout(rng, open_office_span: bool):
    ...
    strips = [(i * W / n_zones, (i + 1) * W / n_zones) for i in range(n_zones)]
    ...
    if open_office_span:
        ...
        for zi, (xs0, xs1) in enumerate(strips):
            for r in _subdivide_strip(xs0, xs1, 0.0, yb0, int(rng.integers(2, 4)), rng):
                rooms.append((*r, zi))
            for r in _subdivide_strip(xs0, xs1, yb1, D, int(rng.integers(2, 4)), rng):
                rooms.append((*r, zi))
    else:
        for zi, (xs0, xs1) in enumerate(strips):
            for r in _subdivide_strip(xs0, xs1, 0.0, D, int(rng.integers(2, 4)), rng):
                rooms.append((*r, zi))
...
def _paste_lighting_glyph(img, glyph_fn, w_px, cpx, cpy, rng):
    ...
    if rng.random() < 0.5:
        tmp = tmp.transpose(rng.choice([Image.FLIP_LEFT_RIGHT, Image.ROTATE_90, Image.ROTATE_270]))
    ...
    for r in rooms:
        ...
        for ci in range(cols):
            for ri_ in range(rows):
                ...
                _paste_lighting_glyph(
                    sh_.img,
                    LIGHTING_GLYPHS[fclass],
...
def _render_mech(rooms, strips, n_zones, W, D, rng):
    ...
    def bar(x0m, y0m, x1m, y1m):
        d.rectangle(
            [
                X(min(x0m, x1m)) - duct_w / 2,
                Y(min(y0m, y1m)) - duct_w / 2,
                X(max(x0m, x1m)) + duct_w / 2,
                Y(max(y0m, y1m)) + duct_w / 2,
            ],
            fill=40,
        )
    ...
    for zi in range(n_zones):
        ...
        for r in rooms:
            ...
            if x0 <= vav_x <= x1 and y0 <= vav_y <= y1:
                components[-len(z_dif) - len(z_sen) - 1]["room_number"] = r["number"]
                break
        zones.append(
            {
                "zone_id": f"Z{zi + 1}",
                ...
                "audit": [f"synthetic zone {zi + 1}: VAV-{zi + 1} serves {len(z_rooms)} rooms"],
            }
        )
...
def _render_elevation(south_windows, grids_v, W, with_grids: bool, px_per_m: float = ELEV_PX_PER_M):
    ...
    if with_grids:
        for lab, gx in grids_v.items():
            u = U(gx)
            d.line([u, V(WALL_H_M + PARAPET_M) - 60, u, V(WALL_H_M + PARAPET_M)], fill=120, width=1)
            d.ellipse(
                [u - 20, V(WALL_H_M + PARAPET_M) - 104, u + 20, V(WALL_H_M + PARAPET_M) - 64],
                outline=0,
                width=2,
            )
            f = _font(22)
            bb = d.textbbox((0, 0), lab, font=f)
            d.text((u - (bb[2] - bb[0]) / 2, V(WALL_H_M + PARAPET_M) - 102), lab, fill=0, font=f)
    ...
    for i, w in enumerate(south_windows):
        ...
        d.text(
            (U((w["s0_m"] + w["s1_m"]) / 2) - (bb[2] - bb[0]) / 2, dy - 22), txt, fill=60, font=f
        )
        wid = f"W{i + 1}"
        win_dets.append(
            {
...
def _south_windows(rooms, D, rng):
    ...
    for r in rooms:
        ...
        for k in range(n):
            ...
            wins.append(
                {"tag": tag, "s0_m": c - ww / 2, "s1_m": c + ww / 2, "room_number": r["number"]}
            )
    ...
    if south_rooms and rng.random() < 0.8:
        ...
        s1 = s0 + ww
        if not any(
            not (s1 <= w["s0_m"] or s0 >= w["s1_m"])
            for w in wins
            if w["room_number"] == r["number"]
        ):
            wins.append({"tag": tag, "s0_m": s0, "s1_m": s1, "room_number": r["number"]})
    return wins
...
def generate_building(seed: int, open_office_span: bool = False) -> dict:
    ...
    gt_links = {
        ...
        "sensor_room": {c["id"]: c["room_number"] for c in components if c["type"] == "sensor"},
        "diffuser_room": {c["id"]: c["room_number"] for c in components if c["type"] == "diffuser"},
        "diffuser_zone": {},
        "sensor_zone": {},
        "window_room": {},
        "zone_rooms": {},
    ...
    bldg = {
        ...
        "sheets": {
            ...
            "lighting": {
                "image": light_img,
                "meta": sheet_meta("light_E101", "lighting_plan", light_origin, PLAN_PX_PER_M),
            },
            "mech": {
                "image": mech_img,
                "meta": sheet_meta("mech_M101", "mech_plan", mech_origin, PLAN_PX_PER_M),
            },
            "elev_grid": {
                "image": elev_g_img,
                "meta": {
                    **sheet_meta("elev_A201", "elevation", (0, 0), ELEV_PX_PER_M),
                    "facade": "south",
            ...
            "elev_nogrid": {
                "image": elev_n_img,
                "meta": {
                    **sheet_meta("elev_A202", "elevation", (0, 0), ELEV_PX_PER_M),
                    "facade": "south",
                    "facade_ref_corner_m": [0.0, D],
                    "facade_length_m": W,
                },
