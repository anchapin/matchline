...
from .sheets import ROOM_NAMES, _font, MIN_ROOM_M
...
TEMPLATE_STUBS = {"vav": True, "ahu": True, "diffuser": False,
                  "grille": False, "sensor": False}
...
def _draw_ahu(d: ImageDraw.ImageDraw, cx, cy, lw: int = 3):
    ...
    d.rectangle([cx - w / 2, cy - h / 2, cx + w / 2, cy + h / 2],
                outline=0, width=lw)
    d.rectangle([cx - w / 2 + 6, cy - h / 2 + 6, cx + w / 2 - 6,
                 cy + h / 2 - 6], outline=0, width=2)
    r = h / 2 - 10
...
_GLYPH_FN = {"vav": _draw_vav, "ahu": _draw_ahu, "diffuser": _draw_diffuser,
             "grille": _draw_grille, "sensor": _draw_sensor}
# Tight symbol extents in meters (template windows / GT bboxes).
_GLYPH_EXT = {"vav": (VAV_W, VAV_H), "ahu": (AHU_W, AHU_H),
              "diffuser": (DIF_S, DIF_S), "grille": (GRI_S, GRI_S),
...
class _Net:
    ...
    def add_piece(self, x1, y1, x2, y2, width_m, duct_id):
        ...
        self.pieces.append({"p1": (float(x1), float(y1)),
                            "p2": (float(x2), float(y2)),
                            "w": width_m, "duct": duct_id})
    ...
    def _cross(p, q):
        ...
        (x3, y3), (x4, y4) = q["p1"], q["p2"]
        vert_p = x1 == x2
        vert_q = x3 == x4
        if vert_p == vert_q:
            return None
        v, h = (p, q) if vert_p else (q, p)
        (vx1, vy1), (vx2, vy2) = v["p1"], v["p2"]
        (hx1, hy1), (hx2, hy2) = h["p1"], h["p2"]
        cx, cy = vx1, hy1
    ...
    def _near_junction(self, pt):
        return any(math.hypot(pt[0] - jx, pt[1] - jy) < 0.05
    ...
    def apply_crossings(self):
            ...
            for j in range(i + 1, len(self.pieces)):
                ...
                if da.startswith("R-"):
                    ...
                    raise AssertionError(
                        f"unexpected supply-supply crossing {da} x {db}")
                self.crossings.append({"x_m": round(pt[0], 3),
                                       "y_m": round(pt[1], 3),
                                       "gapped": gapped["duct"],
                                       "kept": kept["duct"]})
                # gap half-width clears the KEPT duct plus 0.1 m (5 px)
                # so the skeleton cannot bridge through the kept duct
                gaps.setdefault(id(gapped), []).append(
                    (pt, kept["w"] / 2 + 0.10))
        for p in self.pieces:
            ...
            (x1, y1), (x2, y2) = p["p1"], p["p2"]
            vertical = (x1 == x2)
            lo, hi = (min(y1, y2), max(y1, y2)) if vertical else \
                (min(x1, x2), max(x1, x2))
            # (center, half_width) along the piece axis
            centers = sorted(set((round((pt[1] if vertical else pt[0]), 3), hw)
                               for pt, hw in pts))
            ...
            if hi - edge > 0.05:
                segs.append((edge, hi))
            p["sub"] = [((x1, s0), (x1, s1)) if vertical else
                        ((s0, y1), (s1, y1)) for s0, s1 in segs]
...
def _layout_mech(rng):
    ...
    for zi in range(n_vav):
        ...
        if not all(bounds[k + 1] - bounds[k] >= MIN_ROOM_M
                   for k in range(n_r)):
        ...
        for k in range(n_r):
            ...
            room_info.append({
                "id": f"R{ri + 1}", "name": names[ri % len(names)],
                "number": str(201 + ri),
                "rect_m": [x0s, y0, x1s, y1],
                "area_m2": round((x1s - x0s) * (y1 - y0), 2),
                "cx": (x0s + x1s) / 2, "cy": (y0 + y1) / 2})
            grp.append(ri)
            ri += 1
        groups.append(grp)

    y_trunk = D / 2
    ahu = {"id": "AHU-1", "type": "ahu", "x_m": 1.5, "y_m": y_trunk,
           "w_m": AHU_W, "h_m": AHU_H, "tag": "AHU-1",
           "room_id": next(r["id"] for r in room_info
    ...
    for zi, grp in enumerate(groups):
        ...
        for r in zrooms:
            ...
            for ox in offs:
                ...
                dx, dy = r["cx"] + ox, r["cy"]
                dxs.append(dx)
                diffusers.append({"id": f"D{dif_i}", "type": "diffuser",
                                  "x_m": dx, "y_m": dy, "w_m": DIF_S,
                                  "h_m": DIF_S, "tag": f"D{dif_i}",
                                  "room_id": r["id"], "vav": f"VAV-{zi + 1}"})
        ...
        vav = {"id": f"VAV-{zi + 1}", "type": "vav", "x_m": vav_x,
               "y_m": vav_y, "w_m": VAV_W, "h_m": VAV_H,
               "tag": f"VAV-{zi + 1}", "room_id": None}
        vav["room_id"] = next(r["id"] for r in room_info
        ...
        for dx, d in zip(dxs, [d for d in diffusers
                               if d["vav"] == vav["id"]]):
            net.add_junction(dx, vav_y)       # drop meets spine
            net.add_piece(dx, vav_y, dx, d["y_m"], W_DROP,
                          f"S-DROP-{d['id']}")
    ...
    for r in room_info:
        ...
        gx = x0 + 0.15 * (x1 - x0)
        gy = r["cy"] + 0.25 * (y1 - y0)
        grilles.append({"id": f"G{gri_i}", "type": "grille",
                        "x_m": gx, "y_m": gy, "w_m": GRI_S, "h_m": GRI_S,
                        "tag": f"G{gri_i}", "room_id": r["id"]})
    ...
    for r in room_info:
        sen_i += 1
        x0, y0, x1, y1 = r["rect_m"]
        sensors.append({"id": f"T{sen_i}", "type": "sensor",
                        "x_m": (x0 + x1) / 2, "y_m": y0 + 0.35,
                        "w_m": 2 * SEN_R + 0.10, "h_m": 2 * SEN_R + 0.10,
                        "tag": f"T{sen_i}", "room_id": r["id"],
                        "vav": next(v["id"] for zi, v in enumerate(vavs)
                                    if r["id"] in
                                    [room_info[i]["id"] for i in groups[zi]])})

    net.apply_crossings()

    # -- GT graph (supply only) ---------------------------------------------
    nodes, edges = [{"id": "AHU-1", "kind": "ahu",
                     "x_m": ahu["x_m"], "y_m": ahu["y_m"]}], []
    def _add_edge(a, b, duct, pts):
        length = sum(math.hypot(pts[k + 1][0] - pts[k][0],
                                pts[k + 1][1] - pts[k][1])
                     for k in range(len(pts) - 1))
        edges.append({"a": a, "b": b, "duct": duct,
                      "length_m": round(length, 3)})
    jn = 0
    for vav in vavs:
        nodes.append({"id": vav["id"], "kind": "vav", "x_m": vav["x_m"],
                      "y_m": vav["y_m"]})
    ...
    for tx, tid in sorted((v["x_m"], v["id"]) for v in vavs):
        ...
        _add_edge(jid, tid, f"S-TAP-{tid.split('-')[1]}", [(tx, y_trunk),
                                                          (tx, vavs[[v['id'] for v in vavs].index(tid)]["y_m"])])
        prev, px = jid, tx
    for zi, vav in enumerate(vavs):
        ...
        for tx in tees:
            jn += 1
            jid = f"J{jn}"
            nodes.append({"id": jid, "kind": "junction", "x_m": tx,
                          "y_m": vy})
            _add_edge(prev_n, jid, f"S-SPINE-{zi + 1}", [(px, vy), (tx, vy)])
            prev_n, px = jid, tx
        # spine end stub
        sx1 = max(d["x_m"] for d in zdis) + 0.4
        _add_edge(prev_n, f"{vid}-END", f"S-SPINE-{zi + 1}", [(px, vy),
                                                             (sx1, vy)])
        nodes.append({"id": f"{vid}-END", "kind": "spine_end", "x_m": sx1,
                      "y_m": vy})
        for d in zdis:
            jid = next(n["id"] for n in nodes
                       if n["kind"] == "junction" and n["x_m"] == d["x_m"]
                       and abs(n["y_m"] - vy) < 1e-9)
            nodes.append({"id": d["id"], "kind": "diffuser", "x_m": d["x_m"],
                          "y_m": d["y_m"]})
            _add_edge(jid, d["id"], f"S-DROP-{d['id']}",
                      [(d["x_m"], vy), (d["x_m"], d["y_m"])])
    for s in sensors:
        nodes.append({"id": s["id"], "kind": "sensor", "x_m": s["x_m"],
                      "y_m": s["y_m"]})
    ...
    for zi, vav in enumerate(vavs):
        ...
        duct_len = sum(
            ...
            or e["duct"].startswith("S-DROP-") and e["b"] in zdis)
        zones.append({"zone_id": f"Z{zi + 1}", "vav_id": vav["id"],
                      "room_ids": sorted(zrooms),
                      "sensor_ids": sorted(zsen),
                      "diffuser_ids": sorted(zdis),
                      "duct_length_m": round(duct_len, 3)})

    gt = {"seed": None, "px_per_m": PX_PER_M, "W_m": W, "D_m": D,
          "rooms": [{k: r[k] for k in ("id", "name", "number", "rect_m",
                                      "area_m2")} for r in room_info],
          "components": [ahu] + vavs + diffusers + grilles + sensors,
          "ducts": [{"id": p["duct"], "width_m": p["w"],
                     "pieces_m": [[list(s[0]), list(s[1])]
                                  for s in p.get("sub", [(p["p1"], p["p2"])])]}
                    for p in net.pieces],
...
def render_mech_sheet(net, gt, room_info):
    W, D = gt["W_m"], gt["D_m"]
    img = Image.new("L", (int((W + 2 * MARGIN_M) * PX_PER_M),
                          int((D + 2 * MARGIN_M) * PX_PER_M)), 255)
    ...
    for r in room_info:
        ...
        f = _font(18)
        d.text((px0 + 8, py0 + 6), f"{r['number']} {r['name']}", fill=0,
               font=f)
    ...
    for x1, y1, x2, y2, w in net.iter_draw():
        ...
        if abs(px1 - px2) >= abs(py1 - py2):  # horizontal
            d.rectangle([min(px1, px2), min(py1, py2) - hw,
                         max(px1, px2), max(py1, py2) + hw], fill=0)
        else:  # vertical
            d.rectangle([min(px1, px2) - hw, min(py1, py2),
                         max(px1, px2) + hw, max(py1, py2)], fill=0)
...
def render_template(cls: str, margin_px: int = 4, stubs: bool | None = None) -> np.ndarray:
    ...
    ex, ey = {"vav": (40, 40), "ahu": (48, 8), "diffuser": (8, 30),
              "grille": (8, 30), "sensor": (0, 0)}[cls] if stubs else (0, 0)
...
def training_crops_from_sheets(n_per_class: int = 200, seed: int = 0,
                               bg_per_sheet: int = 50):
    ...
    while min(counts.values()) < n_per_class:
        ...
        if s <= 40:
            comp_px = [((c["x_m"] + MARGIN_M) * PX_PER_M,
                        (c["y_m"] + MARGIN_M) * PX_PER_M) for c in comps]
            ...
            while made < bg_per_sheet and tries < 400:
                ...
                if min(math.hypot(bx - qx, by - qy)
                       for qx, qy in comp_px) < 60:
                    continue
